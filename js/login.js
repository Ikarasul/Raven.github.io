document.addEventListener('DOMContentLoaded', () => {
    lucide.createIcons();
    localStorage.removeItem('raven_user'); // Clear prev session

    // Check for guest parameter in URL
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('guest') === 'true') {
        // Auto-trigger guest login
        guestLogin();
    }
    ...
    if (form) {
        form.addEventListener('submit', (e) => {
            ...
            if (u === 'Raven' && p === '12345678123') {
                localStorage.setItem('raven_user', JSON.stringify({ name: 'Raven', role: 'Admin' })); // JSON format consistent with nav.js
...
function togglePass(event) {
    // Determine the button element properly
