(function () {
    ...
    style.innerHTML = `
        /* Chatbot Widget Styles (Cute & Fresh Theme) */
        .chat-widget {
            position: fixed;
            bottom: 1.5rem;
            right: 1.5rem;
        ...
        /* Mobile: Lift chatbot above bottom nav */
    ...
    // Security
    const RATE_LIMIT = 15; // Optimized increase
    const THROTTLE_MS = 1000; // Faster response
    let requestCount = 0;
    ...
    async function checkWeather(text) {
        ...
        try {
            // 1. Get Location (IP)
            let lat = 13.7563, lon = 100.5018, city = "กรุงเทพฯ"; // Default
            try {
    ...
    function checkLocalData(text) {
        ...
        // 2. TASK REMINDER (ฝากงาน...) [AUTH REQUIRED]
        if (t.startsWith('ฝากงาน') || t.startsWith('การบ้าน') || t.toLowerCase().startsWith('task ')) {
            ...
            tasks.unshift({
                id: 'chat_' + Date.now(),
                courseId: 'GEN', // Default to General
                title: taskName,
                done: false,
                file: null
            });
        ...
        // 3. FINANCE (จดรายรับ/จ่าย) [AUTH REQUIRED]
        ...
        if (incomeMatch || expenseMatch) {
            ...
            const amount = isIncome ? rawAmount : -rawAmount; // Fix: Expenses must be negative for P3
            ...
            tx.push({
                id: 'ftx_' + Date.now(),
                desc: desc, // Note: P3 uses 'text' property, let's support both or fix P3 reading
                text: desc, // Fix: P3 uses 'text', add this key for compatibility
                amount: amount,
                type: isIncome ? 'income' : 'expense',
                date: new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) // Fix: P3 format
            });
            localStorage.setItem('raven_finance', JSON.stringify(tx));

            // Notify P3 to update if open
